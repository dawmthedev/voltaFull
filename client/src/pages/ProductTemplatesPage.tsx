...
import { ProductTemplate, ProductType, EventTemplate } from '../types/task';
import { useGetProductTemplatesQuery, useCreateProductTemplateMutation, useUpdateProductTemplateMutation, useDeleteProductTemplateMutation } from '../store/productTemplatesSlice';

const ProductTemplatesPage: React.FC = () => {
  const { data: productTemplates, isLoading, refetch } = useGetProductTemplatesQuery();
  ...
  const handleCreateTemplate = async (newTemplate: Omit<ProductTemplate, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div className="flex items-center mb-4 md:mb-0">
          ...
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Product Templates</h1>
        </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {template.defaultEvents.length} {template.defaultEvents.length === 1 ? 'Event' : 'Events'}
                  </span>
...
const TemplateFormModal: React.FC<TemplateFormModalProps> = ({ template, onClose, onSave }) => {
  ...
  const addEvent = () => {
    ...
    const newEvent: EventTemplate = {
      id: `temp-${Date.now()}`, // Temporary ID for new events
    ...
    setFormData(prev => ({
      ...prev,
      defaultEvents: [...prev.defaultEvents, newEvent],
    }));
  ...
  const removeEvent = (index: number) => {
    setFormData(prev => ({
      ...prev,
      defaultEvents: prev.defaultEvents.filter((_, i) => i !== index),
    }));
  };
                ...
                {formData.defaultEvents.length > 0 ? (
                  <div className="space-y-3 max-h-60 overflow-y-auto p-1">
                    {formData.defaultEvents.map((event, index) => (
                      ...
                      >
                        <div>
                          <h4 className="font-medium text-gray-800 dark:text-white">{event.name}</h4>
                          {event.description && (
