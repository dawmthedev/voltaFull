"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.renderView = void 0;
const TemplateRenderError_js_1 = require("../errors/TemplateRenderError.js");
async function renderView(data, $ctx) {
    const { response, endpoint } = $ctx;
    try {
        const { data } = $ctx;
        const { path, options } = endpoint.view;
        return await response.render(path, { ...options, ...data, $ctx });
    }
    catch (err) {
        throw new TemplateRenderError_js_1.TemplateRenderError(endpoint.targetName, endpoint.propertyKey, err);
    }
}
exports.renderView = renderView;
//# sourceMappingURL=renderView.js.map