...
function lazyRefMapper(jsonLazyRef, options) {
    ...
    if (options.$refs?.find((t) => t === jsonLazyRef.target)) {
        return (0, ref_js_1.createRef)(name, jsonLazyRef.schema, options);
    }
    options.$refs = [...(options.$refs || []), jsonLazyRef.target];
    const schema = jsonLazyRef.getType() && (0, JsonSchemaMapperContainer_js_1.execMapper)("schema", [jsonLazyRef.schema], (0, generics_js_1.mapGenericsOptions)(options));
    return (0, ref_js_1.toRef)(jsonLazyRef.schema, schema, options);
}
exports.lazyRefMapper = lazyRefMapper;
(0, JsonSchemaMapperContainer_js_1.registerJsonSchemaMapper)("lazyRef", lazyRefMapper);
