...
function extractParameters(jsonOperation, options) {
    return jsonOperation
        .get("parameters")
        .filter((parameter) => (0, JsonParameterTypes_js_1.isParameterType)(parameter.get("in")))
        .reduce((inputs, parameter) => {
        const [parameters, bodyParameters] = inputs;
        if ([JsonParameterTypes_js_1.JsonParameterTypes.BODY, JsonParameterTypes_js_1.JsonParameterTypes.FILES].includes(parameter.get("in"))) {
...
function operationMapper(jsonOperation, { tags = [], defaultTags = [], ...options } = {}) {
    const { consumes, produces, ...operation } = (0, JsonSchemaMapperContainer_js_1.execMapper)("map", [jsonOperation], { ...options, ignore: ["parameters"] });
    ...
    if (jsonOperation.get("responses").size === 0) {
        operation.responses = {
            "200": {
                description: (0, httpStatusMessages_js_1.getStatusMessage)(200)
            }
        };
    }
    ...
    operation.parameters = (0, JsonSchemaMapperContainer_js_1.execMapper)("operationInParameters", [parameters], options);
    if (bodyParameters.length) {
        operation.requestBody = (0, JsonSchemaMapperContainer_js_1.execMapper)("operationRequestBody", [bodyParameters], parametersOptions);
    }
...
(0, JsonSchemaMapperContainer_js_1.registerJsonSchemaMapper)("operation", operationMapper);
