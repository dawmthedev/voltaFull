"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getJsonSchema = void 0;
const core_1 = require("@tsed/core");
require("../components/index");
const SpecTypes_js_1 = require("../domain/SpecTypes.js");
const JsonSchemaMapperContainer_js_1 = require("../registries/JsonSchemaMapperContainer.js");
const getJsonEntityStore_js_1 = require("./getJsonEntityStore.js");
/**
 * @ignore
 */
const CACHE_KEY = "$cache:schemes";
/**
 * @ignore
 */
function getKey(options) {
    return JSON.stringify(options);
}
/**
 * @ignore
 */
function get(entity, options) {
    const cache = entity.store.get(CACHE_KEY) || new Map();
    const key = getKey(options);
    if (!cache.has(key)) {
        const schema = (0, JsonSchemaMapperContainer_js_1.execMapper)("schema", [entity.schema], options);
        if (Object.keys((0, core_1.getValue)(options, "components.schemas", {})).length) {
            schema.definitions = options.components.schemas;
        }
        cache.set(key, schema);
    }
    entity.store.set(CACHE_KEY, cache);
    return cache.get(key);
}
function getJsonSchema(model, options = {}) {
    const entity = (0, getJsonEntityStore_js_1.getJsonEntityStore)(model);
    const specType = options.specType || SpecTypes_js_1.SpecTypes.JSON;
    options = {
        endpoint: true,
        groups: [],
        inlineEnums: specType === SpecTypes_js_1.SpecTypes.JSON,
        ...options,
        specType,
        components: {
            schemas: {}
        }
    };
    if (entity.decoratorType === "parameter") {
        options = {
            ...options,
            genericTypes: entity.nestedGenerics[0],
            nestedGenerics: entity.nestedGenerics,
            groups: entity.parameter?.groups
        };
    }
    return get(entity, options);
}
exports.getJsonSchema = getJsonSchema;
//# sourceMappingURL=getJsonSchema.js.map