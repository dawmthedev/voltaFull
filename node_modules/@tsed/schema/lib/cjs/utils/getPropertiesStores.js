...
const alterIgnore_js_1 = require("../hooks/alterIgnore.js");
...
function getPropertiesStores(target) {
    ...
    if (!store.$properties) {
        const stores = (0, getInheritedStores_js_1.getInheritedStores)(store);
...
function getProperties(target, options = {}) {
    ...
    stores.forEach((store, key) => {
        if (!options.withIgnoredProps) {
            if ((0, alterIgnore_js_1.alterIgnore)(store.itemSchema, options)) {
                return;
            }
        }
        map.set(key, store);
