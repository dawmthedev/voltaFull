...
const JsonResponse_js_1 = require("./JsonResponse.js");
class JsonOperation extends JsonMap_js_1.JsonMap {
    ...
    addTags(tags) {
        tags = (0, core_1.uniqBy)([...(this.get("tags") || []), ...tags], "name");
    ...
    isRedirection(status) {
        if (this.#redirection) {
            if (status) {
                return (0, isSuccessStatus_js_1.isRedirectionStatus)(status);
            }
        }
        return this.#redirection;
    }
    addResponse(statusCode, response) {
        if (((0, isSuccessStatus_js_1.isSuccessStatus)(statusCode) || (0, isSuccessStatus_js_1.isRedirectionStatus)(statusCode)) && !this.#status) {
        ...
        if (!currentResponse) {
            ...
            response.forEach((value, key) => {
                if (!["content"].includes(key)) {
                    currentResponse.set(key, (0, core_1.deepMerge)(currentResponse.get(key), value));
                }
            });
    ...
    getResponseOf(status) {
        return (status === "default" ? this.response : this.getResponses().get(String(status))) || new JsonResponse_js_1.JsonResponse();
    }
    ...
    addSecurityScopes(name, scopes) {
        ...
        security[name] = (0, core_1.uniq)([...(security[name] || []), ...scopes]);
    ...
    addProduce(produce) {
        const produces = (0, core_1.uniq)([].concat(this.get("produces"), produce)).filter(Boolean);
