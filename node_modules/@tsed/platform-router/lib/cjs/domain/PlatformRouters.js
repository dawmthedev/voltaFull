...
const useContextHandler_js_1 = require("../utils/useContextHandler.js");
const PlatformLayer_js_1 = require("./PlatformLayer.js");
const PlatformRouter_js_1 = require("./PlatformRouter.js");
let AUTO_INC = 0;
...
function createInjectableRouter(injector, provider) {
    ...
    const router = injector.invoke(PlatformRouter_js_1.PlatformRouter);
...
di_1.GlobalProviders.createRegistry(di_1.ProviderType.CONTROLLER, di_1.ControllerProvider, {
    onInvoke(provider, locals, { injector }) {
        ...
        locals.set(PlatformRouter_js_1.PlatformRouter, router);
    }
});
let PlatformRouters = class PlatformRouters {
    constructor(injector) {
        ...
        this.allowedVerbs = schema_1.OPERATION_HTTP_VERBS;
    }
    prebuild() {
    ...
    from(token, parentMiddlewares = []) {
        ...
        const appendChildrenRoutesFirst = this.injector.settings.get("router.appendChildrenRoutesFirst", false);
        ...
        (0, schema_1.getOperationsRoutes)(provider.token, { allowedVerbs: this.allowedVerbs }).forEach((operationRoute) => {
            ...
            router.addRoute(operationRoute.method, operationRoute.path || "", [
                (0, useContextHandler_js_1.useContextHandler)(($ctx) => {
        ...
        return layers
            ...
            if (layer.router) {
                return this.flatMapLayers(layer.layers).map((subLayer) => {
                    return new PlatformLayer_js_1.PlatformLayer({
                        ...subLayer,
                        path: (0, schema_1.concatPath)(layer.path, subLayer.path)
                    });
                });
            }
            return new PlatformLayer_js_1.PlatformLayer(layer);
        })
            .map((layer) => {
