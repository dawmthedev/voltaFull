...
const formatMethod_js_1 = require("../utils/formatMethod.js");
const PlatformHandlerMetadata_js_1 = require("./PlatformHandlerMetadata.js");
...
let PlatformRouter = PlatformRouter_1 = class PlatformRouter {
        ...
        const layer = handlers.reduce((layer, item) => {
            if ((0, core_1.isString)(item) || item instanceof RegExp) {
                ...
                if (item instanceof PlatformRouter_1) {
                    ...
                    item = PlatformHandlerMetadata_js_1.PlatformHandlerMetadata.from(this.injector, item);
                }
                layer.handlers.push(item);
            }
            return layer;
        }, new PlatformLayer_js_1.PlatformLayer({
            method: "use",
    ...
    addRoute(method, path, handlers, opts = {}) {
        const layer = new PlatformLayer_js_1.PlatformLayer({
            provider: this.provider,
            method: (0, formatMethod_js_1.formatMethod)(method),
            path,
            handlers: handlers.map((input) => {
                return PlatformHandlerMetadata_js_1.PlatformHandlerMetadata.from(this.injector, input, opts);
            }),
            opts
        });
        this.layers.push(layer);
