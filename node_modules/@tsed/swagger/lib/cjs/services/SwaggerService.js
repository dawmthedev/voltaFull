...
const includeRoute_js_1 = require("../utils/includeRoute.js");
const readSpec_js_1 = require("../utils/readSpec.js");
    ...
    async getOpenAPISpec(conf) {
        if (!this.#specs.has(conf.path)) {
            ...
            const tokens = this.platform
                ...
                .filter(({ routes, provider }) => [...routes.values()].some((route) => (0, includeRoute_js_1.includeRoute)(route, provider, conf)))
                .map(({ route, provider }) => ({ token: provider.token, rootPath: route }));
            let spec = (0, schema_1.generateSpec)({
                tokens,
                ...conf,
                fileSpec: specPath ? await (0, readSpec_js_1.readSpec)(specPath) : {},
                version,
                acceptMimes
            });
