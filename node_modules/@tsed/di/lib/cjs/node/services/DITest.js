...
const DIContext_js_1 = require("../domain/DIContext.js");
const setLoggerConfiguration_js_1 = require("../utils/setLoggerConfiguration.js");
/**
...
class DITest {
    ...
    static async createContainer() {
        await DITest.injector.load((0, index_js_1.createContainer)());
    }
    /**
     * Create a new injector with the right default services
     */
    static createInjector(settings = {}) {
        const injector = new index_js_1.InjectorService();
        injector.logger = logger_1.$log;
        // @ts-ignore
        injector.settings.set(DITest.configure(settings));
        (0, setLoggerConfiguration_js_1.setLoggerConfiguration)(injector);
        return injector;
    }
    /**
    ...
    static async invoke(target, providers = []) {
        const locals = new index_js_1.LocalsContainer();
        providers.forEach((p) => {
            locals.set(p.token, p.use);
        });
        locals.set(index_js_1.InjectorService, DITest.injector);
        ...
        if ((0, core_1.isClass)(instance)) {
            await Promise.all(Object.values(instance).filter(core_1.isPromise));
        }
        return instance;
    }
    /**
    ...
    static createDIContext() {
        return new DIContext_js_1.DIContext({
            id: "id",
