...
const EnginesContainer_js_1 = require("../registries/EnginesContainer.js");
...
function getEngine(name) {
    const cb = async (path, options, cb) => {
        const engine = EnginesContainer_js_1.engines.get(name);
        await engine.$onInit();
...
function getEngines() {
    localEngines =
        localEngines ||
            EnginesContainer_js_1.engines.getSupportedEngines().reduce((acc, key) => {
                return {
                    ...acc,
                    [key]: getEngine(key)
                };
