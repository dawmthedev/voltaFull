...
export class AuthenticationController {
  ...
  public async startVerification(@BodyParams() body: StartVerificationParams) {
    const { email, type } = body;
     logger.success("Starting verification", { email, type });
    ...
    const maskedCode = verificationData.code
      ...
      : "";
     logger.success("Generated verification code", { code: maskedCode });
  ...
  public async adminLogin(@BodyParams() body: AdminLoginBody, @Response() res: Response) {
    const { email, password } = body;
     ...
     logger.success("Testing Login...");
     logger.success("Received login request for email:", email); // Be careful with logging sensitive information
  ...
  public async crmNewSales(@BodyParams() body: CrmPayBody, @Response() res: Response) {
    ...
    const headers = {
     ...
     logger.success("Getting CRM Payroll...");
     logger.success(recordId);
    ...
    const NewSaleResults = dataArray.map((record) => ({
     ...
     logger.success("Returning CRM payroll data...");
  ...
  private async analyzeWithOpenAI(buffer: Buffer): Promise<string> {
    ...
    const payload = {
     ...
     logger.success("Sending Request to OpenAI:", { apiUrl, payload }); // Log request details
    ...
    while (attempt < maxRetries) {
      try {
        const response = await axios.post(apiUrl, payload, {
         ...
         logger.success("Received Response from OpenAI:", response.data); // Log response details
        ...
        if (error.response && error.response.status === 429) {
          ...
          attempt++;
           logger.success(`Rate limited. Retrying attempt ${attempt} in ${retryDelay}ms...`);
  ...
  public async crmDealsRookieLedgens(@Response() res: Response) {
    ...
    const headers = {
     ...
     logger.success("Getting CRM users...");
  ...
  public async crmDeal(@BodyParams() body: CrmDealsBody, @Response() res: Response) {
    ...
    const headers = {
     ...
     logger.success("Getting CRM deal...");
  ...
  public async previousPay(@BodyParams() body: CrmDealsBody, @Response() res: Response) {
    ...
    const headers = {
     ...
     logger.success("Getting CRM users Anticipated Pa..");

     logger.success(API_URL);
  ...
  public async crmAvgTimelines(@BodyParams() body: timelineBody, @Response() res: Response) {
    ...
    // Log the original item for debugging
     logger.success("Original Item:", item);
