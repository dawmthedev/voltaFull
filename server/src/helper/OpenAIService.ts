...
export class OpenAIService {
  ...
  public async askQuestion(question: string): Promise<string> {
    try {
      ...
      logger.request("Launching Assistant");
      const assistant = await this.openai.beta.assistants.create({
        name: "Construction Company Project Concierge",
        instructions:
          "You are a personal concierge assistant for a construction company overseeing all project data. Use any resources available to you to consult users about their projects.",
        tools: [{ type: "code_interpreter" }],
        model: "gpt-4-1106-preview"
      });

      logger.request("Creating Thread");
      // Create a thread
      const thread = await this.openai.beta.threads.create();
      logger.request("Passing the user question into thread");
      // Pass in the user question into the existing thread
      await this.openai.beta.threads.messages.create(thread.id, {
        role: "user",
        content: question
      });
      logger.request("Starting assistant and waiting for response...");
      // Start the assistant and wait for the response
      const run = await this.openai.beta.threads.runs.create(thread.id, {
        assistant_id: assistant.id
      });

      logger.request("Poll for the run status until it is completed");

      // Poll for the run status until it's completed
      let runStatus = await this.openai.beta.threads.runs.retrieve(thread.id, run.id);

      logger.request("...run status began...");
      while (runStatus.status !== "completed") {
        await new Promise((resolve) => setTimeout(resolve, 2000));
        runStatus = await this.openai.beta.threads.runs.retrieve(thread.id, run.id);
      }

      logger.request("Returning Messages...");
      const messages = await this.openai.beta.threads.messages.list(thread.id);

      logger.request("Last Message for run...");
